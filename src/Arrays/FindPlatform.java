 ...
 * A Simple Solution is to take every interval one by one and find the number of intervals that overlap
 * with it. Keep track of maximum number of intervals that overlap with an interval. Finally return the
 * maximum value. Time Complexity of this solution is O(n2).
 *
 ...
 *                                               needed at any time
 *                                            = 3
 * Below is the implementation of the above approach. Note that the implementation doesn’t create a
 * single sorted list of all events, rather it individually sorts arr[] and dep[] arrays, and then
 * uses merge process of merge sort to process them together as a single sorted array.
 */
