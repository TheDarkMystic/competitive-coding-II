 ...
 * Check current cell is not already used and character in it matches with the character in the word
 * at index (starts will 0).
 * Check if index = length of the word, means we have found the word. return true and print the
 * solution matrix.
 * If above criteria matches, mark that cell with a number Whenever any cell matches with the criteria,
 * put a number corresponding to it in solution matrix. (start with 0 and keep incrementing it, it will
 * show us the path for the word).
 * Now try to solve rest of the problem recursively by making index +1. Check all 8 directions
 * ( up, down, left right, diagonally up-left, diagonally up-right, diagonally down-left, diagonally
 * down-right). Check the boundary conditions as well
 * If none of the 8 recursive calls return true, BACKTRACK and undo the changes ( put 0 to corresponding
 * cell in solution matrix) and return false.
...
public class WordMatrix {
    ...
    public boolean search(char[][] matrix, String word, int row, int col,
        ...
        // solution[row][col] != 0  means the cell is already explored, no neex to explore again
